...
func TestTop10HarderPositive(t *testing.T) {
	as := assert.New(t)
	//as.EqualValues(
	//	[10]string{"нога", "и", "это", "слова", "разные", "с", "and", "не", "dog", "функцию"},
	res := Top10(`Частотный анализ
...
Эти слова пришлось добить, чтоб было явно 10 слов превосходящих другие по числу совпадений >4:
Нога Нога не and функцию dog`)

	resultContains := func (word string) bool {
		for _, w := range res {
			if word == w {
				return true
			}
	...
	topWords := [10]string{"нога", "и", "это", "слова", "разные", "с", "and", "не", "dog", "функцию"}

	for _, word := range topWords {
		as.True(resultContains(word), "doesn't contain word: " + word)
	}
}
